...
internals.keyCondition = (keyName, schema, query) => {
  ...
  const f = (operator) => {
    ...
    return function () {
      ...
      const existingValueKeys = Object.keys(query.request.ExpressionAttributeValues || {});
...
internals.addExpressionAttributes = (request, condition) => {
  ...
  const expressionAttributeNames = Merge(
    {}, condition.attributeNames, request.ExpressionAttributeNames);
  const expressionAttributeValues = Merge(
    {}, condition.attributeValues, request.ExpressionAttributeValues);
...
Query.prototype.addKeyCondition = function (condition) {
  ...
  if (IsString(this.request.KeyConditionExpression)) {
...
Query.prototype.addFilterCondition = function (condition) {
  ...
  internals.addExpressionAttributes(this.request, condition);
  if (IsString(this.request.FilterExpression)) {
...
Query.prototype.attributes = function (attrs) {
  ...
  const expressionAttributeNames = Reduce(attrs, (result, attr) => {
  ...
  this.request.ExpressionAttributeNames = Merge(
    {}, expressionAttributeNames || {}, this.request.ExpressionAttributeNames || {});
...
Query.prototype.exec = function (callback) {
  ...
  const runQuery = (params, cb) => {

    self.table.runQuery(params, cb);
  };
...
Query.prototype.buildKey = function () {
  ...
  const existingValueKeys = Object.keys(this.request.ExpressionAttributeValues || {});
...
Query.prototype.buildRequest = function () {
  ...
  return Merge({}, this.request, { TableName: this.table.tableName() });
};
