...
internals.findDynamoTypeMetadata = data => {
  const meta = Find(data.meta, data => IsString(data.dynamoType));
...
internals.parseDynamoTypes = data => {
  if (IsPlainObject(data) && data.type === 'object' && IsPlainObject(data.children)) {
    return internals.parseDynamoTypes(data.children);
  }

  const mapped = Reduce(data, (result, val, key) => {
    if (val.type === 'object' && IsPlainObject(val.children)) {
...
const Schema = module.exports = function (config) {
  ...
  Joi.validate(config, internals.configSchema, { context: context }, (err, data) => {
    ...
    if (data.indexes) {
      self.globalIndexes = KeyBy(Filter(data.indexes, { type: 'global' }), 'name');
      self.secondaryIndexes = KeyBy(Filter(data.indexes, { type: 'local' }), 'name');
    }

    if (data.schema) {
      self._modelSchema = IsPlainObject(data.schema) ? Joi.object().keys(data.schema) : data.schema;
    } else {
    ...
    if (self.timestamps) {
      ...
      if (self.createdAt) {
        if (IsString(self.createdAt)) {
          createdAtParamName = self.createdAt;
        }
      }

      if (self.updatedAt) {
        if (IsString(self.updatedAt)) {
...
internals.invokeDefaultFunctions = data => MapValues(data, val => {
