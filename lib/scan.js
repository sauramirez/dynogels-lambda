...
internals.keyCondition = (keyName, schema, scan) => {
  ...
  const f = (operator) => {
    ...
    return function () {
      ...
      const existingValueKeys = Keys(scan.request.ExpressionAttributeValues);
...
Scan.prototype.addFilterCondition = function (condition) {
  ...
  const expressionAttributeNames = Merge({}, condition.attributeNames, this.request.ExpressionAttributeNames);
  const expressionAttributeValues = Merge({}, condition.attributeValues, this.request.ExpressionAttributeValues);
...
Scan.prototype.attributes = function (attrs) {
  ...
  this.request.ProjectionExpression = Keys(expressionAttributeNames).join(',');
  this.request.ExpressionAttributeNames = Merge({}, expressionAttributeNames, this.request.ExpressionAttributeNames);
